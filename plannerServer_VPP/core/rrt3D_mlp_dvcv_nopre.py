  ...
  ## 在上一个NBV一定半径圆内采样位置
    ...
    def sample_direction(self,locations):
        ...
        for i in range(sample_Num):
             ...
             for u in np.linspace(u_center-10/180*np.pi,u_center+10/180*np.pi,3):
                 for v in np.linspace(v_center-20/180*np.pi,v_center+20/180*np.pi,5):
                     if u < -np.pi/2.0:
                         u = -np.pi/2.0
                     if u > np.pi/2.0:
                         u = np.pi/2.0
        ...
        depth_clip[select_depth<1.5]= np.exp(-2*np.abs(select_depth[select_depth<1.5] - 3.5))
        depth_clip[select_depth>4.5] = np.exp(-2*np.abs(select_depth[select_depth>4.5] - 3.5))
        ...
        while self.ind < self.maxiter:
            ...
            if not collide and dist != 0 and xnew[1]>self.ymin and (self.xt not in self.V):
                ...
                if self.ind == self.maxiter-1:
                    self.done = False
                    break
                elif getDist(xnew, self.xt) <= self.stepsize:
